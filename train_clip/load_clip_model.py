...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, clip_finetune= False):
    """Load a CLIP model
    Parameters
    ----------
    name : str
...
def load_clip_by_name(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, build_clip_type=None,clip_finetune=False):
    """Load a CLIP model
    Parameters
    ----------
    name : str
    ...
    if not jit:
        model = build_clip_type(state_dict or model.state_dict()).to(device)
        if str(device) == "cpu":
            model.float()
        return model
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text, pos_embedding=True, text_features=None):
        ...
        return logits_per_image, logits_per_text # [bs,2], [2,bs]
